        ...
        try:
            if self.embedding_type == "huggingface":
                if HFRAGLoader is None:
                    raise ImportError("HuggingFace RAG loader not available. "
                                   "Make sure to install the required dependencies.")
                self.logger.info(f"Initializing HuggingFace embeddings with model: {self.model_name}")
                rag_loader = HFRAGLoader(model_name=self.model_name or "sentence-transformers/all-MiniLM-L6-v2")
                return rag_loader.get_vector_store("data/rag_sources")
            
            elif self.embedding_type == "openai":
                if create_vector_store_from_sources is None:
                    raise ImportError("OpenAI RAG loader not available. "
                                   "Make sure to install the required dependencies.")
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.vector_store, 'as_retriever'):
                    ...
                    retrieved_chunks = self.vector_store.similarity_search(job_description, k=4)
                else:
